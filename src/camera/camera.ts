...
export async function load_camera_presets(file: BlobPart): Promise<CameraPreset[]> {
...
const CANONICAL_FORWARD = vec3.create(0, 0, 1);
...
export class Camera {
  ...
  update_buffer(): void {
    ...
    vec3.transformMat4Upper3x3(CANONICAL_FORWARD, inv_view_matrix, this.look);
    vec3.normalize(this.look, this.look);

    vec3.transformMat4Upper3x3(CANONICAL_RIGHT, inv_view_matrix, this.right);
    vec3.normalize(this.right, this.right);

    vec3.transformMat4Upper3x3(CANONICAL_UP, inv_view_matrix, this.up);
